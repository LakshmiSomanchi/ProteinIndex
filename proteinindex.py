...
# --- Define the tabs ---
# Main dashboard tabs first, then dynamically add tabs for each defined DataFrame
...
for i, tab_name in enumerate(list(dynamic_dfs.keys())):
    with tabs[i + 2]: # Offset by 2 for the first two custom tabs
        st.header(f"Data: '{tab_name}'")
        current_df = dynamic_dfs[tab_name]
        ...
        st.download_button(
            ...
            mime='text/csv',
            help=f"Download the data from the '{tab_name}' tab."
        )
